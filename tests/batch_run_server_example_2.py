...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  3 09:36:55 2023

@author: Nishanth Anandanadarajah <anandanadarajn2@nih.gov> allrights reserved

this script is run to obtain the over all pipe-line upto outlier detection in one part
modified  on Tue Jul 23 19:35:24 2024
to run the python installed with whl

"""

'''
To insall through via whl before register just using this temporary

cd .../EEG_PSG_loose_lead_test/dist/
pip install sleep_EEG_loose_lead_detect-0.0-py3-none-any.whl

modified on Thu Aug  1 08:07:51 2024
to add timeout error (here used 5 minutes)-line  -->>   signal.alarm(300)
based on the system or server this need to be changed
'''
import os
import sys
import logging
...
# --------------------------------------------------------------------------
# add time out exception handler
# --------------------------------------------------------------------------
import signal
...
def EEG_sleep_loose_lead(in_name_temp):
    '''
    Parameters
    ----------
    in_name_temp : str
    ...
    # Set the timeout alarm for 5 minutes (300 seconds)
    # --------------------------------------------------------------------------
    signal.signal(signal.SIGALRM, timeout_handler)
    signal.alarm(300)

    opt_paramters = parameter_assignment()
    opt_paramters.GUI_percentile = False
    '''
    ...
    # --------------------------------------------------------------------------
    loading_dir_pre.in_loc = 'assign/full_eeg/'
    loading_dir_pre.out_loc = 'assign/check_loose_lead/full_eeg/threh_'+str(amplitude_thres)+'/'
    ...
    loading_dir_pre.keep_signature_dic['out_loc_outlier']=False
    loading_dir_pre.keep_signature_dic['sleep_anot']=False
    loading_dir_pre.keep_signature_dic['MT_spec']=False
    loading_dir_pre.keep_signature_dic['annota_NREM_REM']=False
    loading_dir_pre.keep_signature_dic['splidle_inc']=False
    loading_dir_pre.keep_signature_dic['tex_files']=False
    
    loading_dir_pre.save_spindle_loc = False
    # --------------------------------------------------------------------------
    # re assign the loading_dir_pre object
    # --------------------------------------------------------------------------
    loading_dir_pre.assign_directories()
    loading_dir_pre.bad_events_file_path ='/ddn/gs1/home/anandanadarajn2/Sleep_subject_categories/bad_events/bad_events_bathroom.txt'
    # loading_dir_pre.bad_epochs_folder = '/ddn/gs1/home/talukdera2/eeg_dl/data/processed_data/v2/bad_epochs'
    ...
    # --------------------------------------------------------------------------
    # the first try block check for the timout exception
    # --------------------------------------------------------------------------
    try:
        
        # --------------------------------------------------------------------------
        # the second try block check for the normal exception
        # --------------------------------------------------------------------------
        try:
            # --------------------------------------------------------------------------
            # since we already assigned the directories
            #  just use them to load it
            # --------------------------------------------------------------------------
            ...
            epoch_length, line_freq, bandpass_freq, normal_only, notch_freq_essential_checker, amplitude_thres_not_used = opt_paramters.preprocess_par()
            ...
            EEG_root, sleep_stages_or, EEG_channels, Fs, start_time_idx,  sel_index_sl_st, whole_annotations, ev_or = load_root_dataset(in_edf, epoch_length, GUI_percentile=opt_paramters.GUI_percentile)
            ...
            # --------------------------------------------------------------------------
            break_spindle_flatten, break_flat_MT_flatten, z_transform,  inter_mediate_transform,  Fisher_based, flat_MT_consider =opt_paramters.intial_parameters_outlier_vertical_spikes(flat_MT_consider=opt_paramters.flat_MT_consider)
            ...
            # --------------------------------------------------------------------------
            # if loading_dir_pre.keep_signature_dic['evtxt']:
            ...
            while (re_iter_count==0) or (len(loose_lead_channels)< (len(ch_names)-2) and len(loose_lead_channels_old)!=len(loose_lead_channels)):
                ...
                # --------------------------------------------------------------------------
                print("re_iter_count-",in_name_temp,":codes/Jul_29_2024/EEG_PSG_loose_lead_test/tests/run_server_check_events_vin_sp0.py ",re_iter_count)
                if sep:
                    # --------------------------------------------------------------------------
                ...
                apply_conv_window=apply_conv_window, thresh_min_conv=thresh_min_conv, thresh_in_sec=thresh_in_sec, outlier_presene_con_lenth_th=outlier_presene_con_lenth_th,
                        loose_check_with_fill_period=with_fill_period,  len_period_tol_min=5,
            ...
            if np.sum(arousal_annot_NREM_REM_re)>0:
                # and loading_dir_pre.keep_signature_dic['evtxt']:
                # --------------------------------------------------------------------------
                ...
                ep_loose_lead, ep_loose_lead_dur,even_id_unique = main_loose_lead_to_origin_events_dic(arousal_annot_NREM_REM_re, sel_index_sl_st, cont_EEG_segments_np,   
                                                ...
                                                with_fill_period=with_fill_period,  len_period_tol_min=5/60, show_single_outliers_before_combine_tol=True)
        ...
        # --------------------------------------------------------------------------
        # Disable the alarm to avoid the interuption caused by the setted alarm
        # --------------------------------------------------------------------------
        signal.alarm(0)
...
if __name__ == '__main__':
    ...
    # --------------------------------------------------------------------------
    loading_dir_pre.in_loc = 'assign/full_eeg/'
    loading_dir_pre.out_loc = 'assign/check_loose_lead/'    
    # --------------------------------------------------------------------------
    #  to run the multuiple edf files
    # if it saved in pickles that can be loaded via
    # --------------------------------------------------------------------------
    os.chdir('/')
    with open(loading_dir_pre.out_loc+ "load_subjects.pickle", 'rb') as handle:
        subjects = pickle.load(handle)
    del handle
    processes=8

    pool = Pool(processes=processes)  # start avilable processors
    pool.map(EEG_sleep_loose_lead, subjects)
